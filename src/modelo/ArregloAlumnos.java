...
public class ArregloAlumnos implements IColecciones, Serializable  {
    ...
    public Object[][] getDatos() {
        ...
        if (!isVacio()) {
            for (int i = 0; i < this.indice; i++) {
               /* if (this.AlumnoA[i] == null) {
                    resultado[i][0] = this.AlumnoA[i];
                } else {
                    resultado[i][0] = this.AlumnoA[i].getCodigo();
                }*/ 
                resultado[i][0] = this.AlumnoA[i].getCodigo();
                resultado[i][1] = this.AlumnoA[i].getNombres();
                resultado[i][2] = this.AlumnoA[i].getApellidos();
                resultado[i][3] = this.AlumnoA[i].getDNI();
                resultado[i][4] = this.AlumnoA[i].getCorreo();
                resultado[i][5] = this.AlumnoA[i].getFacultad().getNombrefacultad();
